...
contract MerkleAirdrop is EIP712{
    ...
    bytes32 private constant MESSAGE_TYPEHASH = keccak256("AidriopClaim(address account,uint256 amount)");
    ...
    function getMessage(address account, uint256 amount) public view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(abi.encode(MESSAGE_TYPEHASH, AidropClaim({account: account, amount: amount})))
        );
    }
